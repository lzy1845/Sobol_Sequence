...
static void MatMatModM (double A[3][3], double B[3][3], double C[3][3], double m)
   ...
   double V[3], W[3][3];
   for (i = 0; i <= 2;  ++i) {
      for (j = 0; j <= 2;  ++j)  V[j] = B[j][i];
      MatVecModM (A, V, V, m);
      for (j = 0; j <= 2;  ++j)  W[j][i] = V[j];
      }
   for (i = 0; i <= 2;  ++i) {
      for (j = 0; j <= 2;  ++j) 
          C[i][j] = W[i][j];
...
static double U01 (RngStream g)
    ...
    g->Cg[0] = g->Cg[1];   g->Cg[1] = g->Cg[2];   g->Cg[2] = p1;
    /* Component 2 */
    p2 = a21 * g->Cg[5] - a23n * g->Cg[3];
    k  = p2 / m2;  p2 -= k * m2;   if (p2 < 0.0) p2 += m2;
    g->Cg[3] = g->Cg[4];   g->Cg[4] = g->Cg[5];   g->Cg[5] = p2;
    /* Combination */
...
void Rand_CreateStream (RngStream *pg,  char name[32])
   ...
   if (!g) { printf ("CreateStream: No more memory\n\n");  exit (1); }
   g->anti = 0;
   for (i = 0; i < 32;  ++i)  g->name[i] = name[i];
   if (First)      /* the first stream */
      {
      for (i = 0; i < 6;  ++i)  V[i] = (ulongint) defseed[i];
      Rand_SetSeed (g, V);
      for (i = 0; i < 6;  ++i)  lastseed[i] = defseed[i];
      First = 0;
      }
   else
      {
      ...
      for (i = 0; i < 6;  ++i)  V[i] = (ulongint) lastseed[i];
      Rand_SetSeed (g, V);
      }
   *pg = g;
   }
...
void Rand_SetPackageSeed (ulongint seed[6])
   ...
   for (i = 0; i <= 5;  ++i)  lastseed[i] = defseed[i] = seed[i];
   First = 1;
